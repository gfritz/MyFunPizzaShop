...
// server side use of Content Security Policy "CSP"
// a restriction policy, this is optional, it's only about 6% of sites **: MDN says?
// gets annoying when you import 3rd party js at runtime instead of from bundle.
...
let styleSrc =
    [|
        ...
        // only using bootstrap for some colors
        """https://cdn.jsdelivr.net/npm/bootstrap@4.0.0/dist/css/bootstrap.min.css"""
        """https://unpkg.com/leaflet@1.9.4/dist/leaflet.css"""
    |]
    |> String.concat " "

// safari and chrome has different preferences vs src vs srcElem, so we do both
let styleSrcElem =
    [|
        """https://unpkg.com/open-props@1.5.9/open-props.min.css"""
        """https://cdn.jsdelivr.net/npm/bootstrap@4.0.0/dist/css/bootstrap.min.css"""
...
let headerMiddleware = fun (context: HttpContext) (next: Func<Task>) ->
    ...
    // if broswer sniffs to read content to find what it is, then it can
    headers.Add("X-Content-Type-Options", "nosniff")

    match context.Request.Headers.TryGetValue("Accept") with
    | true, accept ->
        if accept |> Seq.exists (fun x -> x.Contains "text/html") then
            // "corp" - optional, confusing; in short, helps to stop opening iframes; spectre mitigation
        ...
        headers.Add(
            ...
            // default-src 'none' most secure setting
            // then 'self' means we can use things from what we say is ourself
            // 'wasm-unsafe-eval' - for lighthouse dev integration
            $"default-src 'none';\
            font-src 'self';\
...
// add in file types that are not available by default
            ...
            fun (context) ->
                ...
                headers.CacheControl <-
                    Microsoft.Net.Http.Headers.CacheControlHeaderValue(
                        Public = true,
                        // this should be longer than 1 day, but then you have to figure out cache-busting
